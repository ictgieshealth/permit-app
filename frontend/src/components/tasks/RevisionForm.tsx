...
import { Task, TaskRevisionRequest } from "@/types/task";
...
export default function RevisionForm({ code }: RevisionFormProps) {
  ...
  const loadTaskData = async () => {
    ...
    try {
      ...
      if (taskData.revision) {
        setFormData({ revision: taskData.revision });
      }
    } catch (err: any) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setLoading(true);
    try {
      const data: TaskRevisionRequest = {
        revision: formData.revision,
      };

      await taskService.setRevision(task.id, data, files);
      router.push("/tasks");
    } catch (err: any) {
  ...
  return (
    <div className="max-w-4xl p-6 mx-auto">
      ...
      <div className="p-6 bg-white border border-gray-200 rounded-lg dark:bg-gray-dark dark:border-gray-800">
        ...
        {task?.reason && (
          <div className="p-4 mb-6 border rounded-lg bg-yellow-50 border-yellow-200 dark:bg-yellow-900/20">
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div>
            <Label>
              Revision Description <span className="text-error-500">*</span>
            </Label>
            <textarea
              name="revision"
              ...
              required
              rows={8}
              placeholder="Describe what has been revised based on the feedback"
          ...
          <div>
            <Label>
              Attachments (Revised files/screenshots)
            </Label>
            <input
              type="file"
              multiple
