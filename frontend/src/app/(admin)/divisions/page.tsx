...
export default function DivisionsPage() {
  ...
  const [deleteModal, setDeleteModal] = useState<{
    ...
    divisionName: string;
  }>({ isOpen: false, divisionId: null, divisionName: "" });
  ...
  const loadDivisions = async () => {
    try {
      ...
      const response = await divisionService.getAll({
        ...
        domain_id: filters.domain_id ? parseInt(filters.domain_id) : undefined,
  ...
  const handleDelete = (id: number, name: string) => {
    setDeleteModal({ isOpen: true, divisionId: id, divisionName: name });
  };
  ...
  const confirmDelete = async () => {
    ...
    setDeleting(true);
    try {
      await divisionService.delete(deleteModal.divisionId);
      setDeleteModal({ isOpen: false, divisionId: null, divisionName: "" });
      loadDivisions();
    } catch (err: any) {
  ...
  return (
    <div className="p-6">
      ...
      <div className="p-4 mb-6 bg-white border border-gray-200 rounded-lg dark:bg-gray-dark dark:border-gray-800">
        <div className="grid grid-cols-1 gap-4 md:grid-cols-4">
          ...
          <div className="flex items-end">
            <Button
              onClick={() => {
                setFilters({ domain_id: "", code: "", name: "" });
                setPagination({ ...pagination, page: 1 });
              }}
            ...
            <tbody className="divide-y divide-gray-200 dark:divide-gray-800">
              {loading ? (
                ...
                divisions.map((division) => (
                  <tr key={division.id} className="hover:bg-gray-50 dark:hover:bg-gray-800/50">
      ...
      <ConfirmModal
        ...
        onClose={() => setDeleteModal({ isOpen: false, divisionId: null, divisionName: "" })}
        onConfirm={confirmDelete}
        title="Delete Division"
        message={`Are you sure you want to delete "${deleteModal.divisionName}"? This action cannot be undone.`}
        confirmText="Delete"
        cancelText="Cancel"
        type="danger"
        loading={deleting}
